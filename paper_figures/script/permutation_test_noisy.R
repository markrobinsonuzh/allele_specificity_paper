...
## Load data
sim_ASM_score_martix_noisy<- readRDS("../data/sim_ASM_score_matrix_noisy.rds")

## Remove positions where all adenoma (i.e. our simulated treatment) samples or all normal samples have NA values
w1 <- which(is.na(sim_ASM_score_martix_noisy[,"adenoma1"])&is.na(sim_ASM_score_martix_noisy[,"adenoma2"])&is.na(sim_ASM_score_martix_noisy[,"adenoma3"]))
w2 <- which(is.na(sim_ASM_score_martix_noisy[,"normal1"])&is.na(sim_ASM_score_martix_noisy[,"normal2"])&is.na(sim_ASM_score_martix_noisy[,"normal3"]))
sm <- sim_ASM_score_martix_noisy[c(-w1,-w2),] 
...
b <- boxplot(a)
b$names <- c("perm", "real")
pdf("../figures/permutation_test_noisy_boxplot.pdf", w=10,h=8)
bxp(b)
abline(a=log(104, base=10),b=0,col='red')
dev.off()
